...
function closeFailPopUp() {
   ...
   openReasonModalFailOLD(); // You should pass any required parameters to openReasonModalFailOLD if needed
}
...
if (closeSymbolFail) {
   closeSymbolFail.addEventListener("click", function () {
      ...
      closeReasonModalFailOLD();
   });
}

function closeReasonModalFailOLD() {
...
//Fail and fail
let reasonFail;
function openReasonModalFailOLD(status) {
...
function submitStatusAndReason(status, reasonFail, applicantId) {
   ...
   xhr.open("POST", "../../../php/updateReasonFailOLD.php", true);
