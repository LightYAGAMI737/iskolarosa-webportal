...
<html lang="en">
   ...
   <div class="left-section">
      <form action="#" id="post-form" method="post" enctype="multipart/form-data">
         ...
         <div class="input-row">
            <input type="text" id="postTitle" name="post_title" minlength="5" maxlength="100" style="margin-right: 10px;" oninput="formatInput(this)" required value="<?php echo isset($postData['post_title']) ? $postData['post_title'] : ''; ?>">
            <select id="tag" name="tag">
               <option value="ceap" <?php echo (isset($postData['tag']) && $postData['tag'] === 'ceap') ? 'selected' : ''; ?>>CEAP</option>
               <option value="lppp" <?php echo (isset($postData['tag']) && $postData['tag'] === 'lppp') ? 'selected' : ''; ?>>LPPP</option>
            </select>
         </div>
         <label for="description" class="required">Description:</label>
         <textarea id="postDescription" name="post_description" rows="4" minlength="10" maxlength="15000" oninput="formatInput(this)" required><?php echo isset($postData['post_description']) ? $postData['post_description'] : ''; ?></textarea>
         <br>
      </div>
      <div class="right-section">
         <label for="image">Upload Picture (Optional, JPG 5MB)</label>
         <input type="file" id="imagePath" name="post_image" accept=".jpg, .jpeg" onchange="validateFile()">
         <br>
         <br>
         ...
         <input type="datetime-local" id="scheduled_at" name="post_schedule_at" onkeydown="preventInput(event)"
               ...
               value="<?php echo isset($postData['post_schedule_at']) ? $postData['post_schedule_at'] : ''; ?>">
